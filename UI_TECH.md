...
## Routing
- Framework: React Router (`react-router-dom`) with `clientLoader` and `clientAction` for business logic.
- Routes:
...
- Storybook:
  ...
  \```
- Vitest with MSW:
  - Write unit tests for components and services, using MSW to mock API responses.
  \```
  ...
  test("fetches recommendations", async () => {
    server.use(
      rest.get("/api/recommendations", (req, res, ctx) =>
        res(ctx.json([{ id: "1", title: "Test Episode" }]))
      )
    );
    const recommendations = await getRecommendations();
